...
<template>
	<div class="bug-report">
		<h2>{{ t('cloud_py_api', 'Bug report') }}</h2>
		<p>{{ t('cloud_py_api', 'Collect not sensitive system info for bug report') }}</p>
		<div class="actions">
			<button v-if="!loading" style="margin: 10px 0;" @click="collectSystemInfo">
				{{ t('cloud_py_api', 'Collect system info') }}
			</button>
			<button v-else style="margin: 10px 0;" disabled>
				<span class="icon-loading" />
			</button>
			<button v-if="systemInfo" @click="copySystemInfoToClipboard">
				{{ t('cloud_py_api', 'Copy to clipboard') }}
			</button>
		</div>
...
export default {
	...
	methods: {
		collectSystemInfo() {
			this.loading = true
			axios.get(generateUrl('/apps/cloud_py_api/api/v1/system-info')).then(res => {
		...
		copySystemInfoToClipboard() {
			...
			showSuccess(this.t('cloud_py_api', 'Copied to clipboard'))
		},
	},
}
</script>
